...
function ToDoList() {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-4 px-4 md:px-0 md:py-8 fixed bottom-0 md:sticky md:top-[3.75rem] z-10 self-center w-full">
        <div className="w-full order-2 md:order-1">
          <Tabs value={selectedStatus} onValueChange={onChangeStatus}>
            <TabsList className="w-full bg-purple-500 rounded-2xl flex">
              {taskStatus.map((task) => (
                <TabsTrigger
                  key={task.value}
                  className="hover:cursor-pointer rounded-2xl text-white data-[state=active]:text-purple-500 flex-1"
                  value={task.value}
                >
                  {task.label}
                </TabsTrigger>
        ...
        <Button
          className="hover:cursor-pointer w-auto bg-purple-500 hover:bg-purple-400 rounded-full md:rounded-2xl self-end md:self-auto order-1 md:order-2"
      ...
      <div className="grid gap-4 overflow-y-auto order-1 md:order-2 mt-8 md:mt-0 md:mb-8">
        {isLoading ? (
        ...
        `}
            >
              ...
              <CardFooter className="flex justify-end gap-2">
                ...
                >
                  <SelectTrigger
                    className={`w-full hover:cursor-pointer rounded-2xl           
                      ${task.status === "pending"
                        ? "bg-yellow-300 text-neutral-50 font-semibold hover:bg-yellow-200"
                        : task.status === "in_progress"
                        ? "bg-sky-300 text-neutral-50 font-semibold hover:bg-sky-200"
                        : "bg-green-300 text-neutral-50 font-semibold hover:bg-green-200"
                    }`}
                  >
                    <SelectValue />
                  </SelectTrigger>
                ...
                <Button
                  className="hover:cursor-pointer bg-purple-500 hover:bg-purple-400 rounded-2xl"
                  size="sm"
